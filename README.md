...
Este proyecto permite entrenar un modelo capaz de identificar la emoción y la persona en imágenes faciales. El modelo es multi-salida: predice tanto la emoción (5 clases) como la persona (2 clases). Incluye scripts para entrenamiento, pruebas individuales (GUI) y pruebas por lotes.
...
1. Organiza tus imágenes en la carpeta `images/`, con subcarpetas para cada emoción (`alegre`, `triste`, `ira`, `sorprendido`, `neutral`).
2. Nombra los archivos de imagen como `Andres_*.jpg` o `Heirismar_*.jpg` según la persona.
