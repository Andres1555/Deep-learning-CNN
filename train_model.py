...
EMOTION_CLASSES = ['alegre', 'triste', 'ira', 'sorprendido', 'neutral']
PERSON_CLASSES = ['Andres', 'Heirismar']
    ...
    for emotion_folder in os.listdir(DATA_DIR):
        ...
        for img_file in os.listdir(emotion_path):
            try:
                ...
                if img_file.lower().startswith('andres_'):
                    person_label = person_map['Andres']
                elif img_file.lower().startswith('heirismar_'):
                    person_label = person_map['Heirismar']
                else:
...
if __name__ == "__main__":
    ...
    if len(images) == 0:
        ...
        X_train, X_val, y_emotion_train, y_emotion_val, y_person_train, y_person_val = train_test_split(
            images, y_emotion, y_person, test_size=0.3, random_state=42
        )
        ...
        report_emotion = classification_report(y_true_emotion, y_pred_emotion, labels=sorted(set(y_true_emotion)), target_names=[EMOTION_CLASSES[i] for i in sorted(set(y_true_emotion))])
        ...
        report_person = classification_report(y_true_person, y_pred_person, labels=sorted(set(y_true_person)), target_names=[PERSON_CLASSES[i] for i in sorted(set(y_true_person))])
